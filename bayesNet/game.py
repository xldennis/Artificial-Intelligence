        ...
        for isPacman, pos in layout.agentPositions:
            ...
            if isPacman:
                ...
                if numGhosts == numGhostAgents:
                    ...
                    if ghostPowers != []: #powers were passed in manually
                        if(numGhosts >= len(ghostPowers)):
                            powers = ghostPowers[0] #if numGhosts > number of powers passed in, assign default power to reamining ghosts
                        else:
    ...
    def initializeAgentObservedVars(self):
        for i in range(len(self.agents)):
            ...
            if s.isPacman:
                ...
                # assign observed vars for each agent independently
                power = None
                if(s.powers.laser):
                    power = 'laser'
                elif(s.powers.blast):
                    power = 'blast'
                elif (s.powers.timestepsBetweenMoves < 1) :
                    power = 'speed'

                observedVarsAssignmentDict = self.pbNet.instantiateObservableVars(power)
                gSize = 1 - 0.2*(observedVarsAssignmentDict['Size']=='small')
                belt = (observedVarsAssignmentDict['Belt']=='1')
